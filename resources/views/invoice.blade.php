    ...
    <div class="flex h-screen w-full items-center justify-center bg-gray-50">
        <div class="w-80 rounded bg-gray-50 px-6 pt-8 shadow">
          ...
          <div class="flex flex-col items-center justify-center gap-2">
            <h4 class="font-semibold">2nd Chance</h4>
          </div>
          <div class="flex flex-col gap-3 border-b py-6 text-xs">
            <p class="flex justify-between">
              <span class="text-gray-400">Receipt No.:</span>
              <span>#{{ $dtcheckout->id }}</span>
            </p>
            <p class="flex justify-between">
              <span class="text-gray-400">status pembayaran :</span>
              <span>{{ $dtcheckout->status }}</span>
            </p>
            <p class="flex justify-between">
              <span class="text-gray-400">Customer:</span>
              <span>{{ $dtcheckout->name }}</span>
            </p>
          </div>
          <div class="flex flex-col gap-3 pb-6 pt-2 text-xs">
            <table class="w-full text-left">
              <thead>
                <tr class="flex">
                  ...
                  <th class="pl-10 py-2">Total</th>
                </tr>
              </thead>
              <tbody>
                <tr class="flex">
                  <td class="flex-1 py-1">{{ $dtcheckout->nama_produk }}</td>
                  <td class="">1</td>
                  <td class="pl-8">Rp.{{ $dtcheckout->harga_produk}}</td>
                </tr>
              </tbody>
            </table>
            ...
            <table class="w-full text-right">
              <tbody>
                <tr class="flex">
                  <th class="w-full py-1">Harga Barang:</th>
                  <th class="py-1 pl-2">Rp.{{ $dtcheckout->harga_produk }}</th>
                </tr>

                <tr class="flex">
                  <th class="w-full py-1">Ongkos Pengiriman:</th>
                  <th class="py-1 pl-2">Rp.{{ $dtcheckout->ongkos}}</th>
                </tr></th>
                </tr>
                <tr class="flex">
                  <th class="w-full py-1">Subtotal pembayaran:</th>
                  <th class="py-1 pl-2">Rp.{{ $dtcheckout->subtotal }}</th>
                </tr></th>
                </tr>
              </tbody>
            </table>
